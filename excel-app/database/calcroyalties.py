...
class ProcessRoyalties(object):
    ...
    def process_one(self, well_id, prod_month, product):
        ...
        well_array = db.select1('Well', ID=well_id)
        well = well_array[0]
        royalty_array = db.select1('Royaltymaster', ID=well.LeaseID)
        royalty = royalty_array[0]
        lease_array = db.select1('Lease', ID=well.LeaseID)
        lease = lease_array[0]
        calc_array = db.select1('Calc', WellID=well_id, ProdMonth = prod_month)
        calc = calc_array[0]
        monthly_array = db.select1('Monthly', WellID = well_id, ProdMonth = prod_month, Product = product)
        monthly = monthly_array[0]
        calc(well, royalty, lease, calc, monthly)
    ...
    def process_all(self):
        ...
        log = open(config.get_temp_dir() + 'log.txt','w')
        log.write ("Hello World.\n")
        for monthlyData in db.Monthly():
            print('**** Processing ***', monthlyData.WellID)
            process_one(monthlyData.wellID, monthlyData.ProdMonth, monthlyData.Product)
